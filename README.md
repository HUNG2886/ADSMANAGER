...
- Website login (`/login`) dùng bảng `User`, mật khẩu Argon2id, `AUTH_SECRET` và cookie phiên mã hoá.
...
- Sau OAuth, hệ thống gọi `customers:listAccessibleCustomers`, đọc `customer_client` đệ quy, lưu hierarchy và đặt đúng `login-customer-id` khi gọi qua MCC.
...
2. Thêm `AUTH_SECRET`, `DEFAULT_ADMIN_EMAIL`, `DEFAULT_ADMIN_PASSWORD`, `DEFAULT_STAFF_EMAIL`, `DEFAULT_STAFF_PASSWORD`, `DATABASE_URL`, `DATABASE_SCHEMA=adsmanager` và `NEXTAUTH_URL`.
...
4. Trong Google Cloud Console, thêm redirect URI chính xác: `${NEXTAUTH_URL}/api/auth/google-ads/callback`.
