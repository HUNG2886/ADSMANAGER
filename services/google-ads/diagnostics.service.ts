...
export async function diagnoseGoogleAdsConnection(connectionId:string,userId:string){
  ...
  try{
    ...
    return{...base,success:true,partialSuccess:hierarchy.issues.length>0,refreshToken:{...base.refreshToken,refreshSucceeded:true},developerTokenAccessLevel:inferDeveloperTokenAccess(null),cloudProject:cloudProjectAssessment(null),api:{version:GOOGLE_ADS_API_VERSION,accessibleCustomerIds:hierarchy.accessibleCustomerIds,mccs:hierarchy.mccs.map(item=>({customerId:item.customerId,parentCustomerId:item.parentCustomerId,manager:item.manager,level:item.level,loginCustomerId:item.manager?item.loginCustomerId:null,testAccount:item.testAccount})),accounts:hierarchy.accounts.map(item=>({customerId:item.customerId,parentCustomerId:item.parentCustomerId,parentManagerCustomerId:item.parentManagerCustomerId,loginCustomerId:item.parentCustomerId?item.loginCustomerId:null,testAccount:item.testAccount})),issues:hierarchy.issues}};
  }catch(cause){
    ...
    return{...base,success:false,partialSuccess:false,refreshToken:{...base.refreshToken,refreshSucceeded},developerTokenAccessLevel:inferDeveloperTokenAccess(error),cloudProject:cloudProjectAssessment(error),api:{version:GOOGLE_ADS_API_VERSION,accessibleCustomerIds,mccs:[],accounts:[],issues:[]},error:googleAdsErrorDetails(error)};
  }
}
