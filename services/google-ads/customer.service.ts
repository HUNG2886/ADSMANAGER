...
export class CustomerService {
  ...
  async listImmediateClients(managerCustomerId: string) {
    ...
    const rows:GoogleCustomerClient[]=[];let pageToken:string|undefined;
    do{
      const payload:SearchResult<{customerClient?:GoogleCustomerClient}>=await this.client.request(`/customers/${id}/googleAds:search`,{method:'POST',body:JSON.stringify({query:'SELECT customer_client.id, customer_client.client_customer, customer_client.descriptive_name, customer_client.currency_code, customer_client.time_zone, customer_client.status, customer_client.manager, customer_client.level, customer_client.test_account FROM customer_client WHERE customer_client.level <= 1',pageSize:10_000,pageToken})});
      rows.push(...(payload.results??[]).map(item=>item.customerClient).filter((item):item is GoogleCustomerClient=>Boolean(item)));
