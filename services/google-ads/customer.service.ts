...
export class CustomerService {
  ...
  async listImmediateClients(managerCustomerId: string) {
    ...
    const rows: GoogleCustomerClient[] = [];
    let pageToken: string | undefined;

    do {
      const payload: SearchResult<{ customerClient?: GoogleCustomerClient }> = await this.client.request(
      ...
      rows.push(
        ...(payload.results ?? [])
          ...
          .filter((item): item is GoogleCustomerClient => Boolean(item)),
      );
