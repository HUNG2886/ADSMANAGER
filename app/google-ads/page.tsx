...
function readableConnectionError(value:string){return value.includes('DEVELOPER_TOKEN_NOT_APPROVED')?'Developer Token đang ở mức Test; kết nối OAuth vẫn hoạt động nhưng chưa thể đọc tài khoản production.':value}
...
export default async function ConnectionsPage({searchParams}:{searchParams:Promise<{connected?:string;error?:string;errorType?:string;errorMessage?:string;requestId?:string;mcc?:string;accounts?:string}>}){
  ...
  return <><div className="ga-page-head"><div><p>GOOGLE ADS</p><h1>Connections</h1><span>Kết nối Gmail thực tế bằng OAuth; không nhập mật khẩu Google vào website.</span></div>{user.role==='ADMIN'&&config.configured&&<a className="ga-primary" href="/api/auth/google-ads"><Plus size={15}/>Đăng nhập Google Ads</a>}</div>
    ...
    {query.error&&<div className="ga-alert danger"><AlertTriangle size={19}/><div><strong>Kết nối chưa hoàn tất</strong><p>{query.errorMessage||errorMessages[query.error]||`Lỗi: ${query.error}`}</p>{query.errorType&&<code>{`Code: ${query.error}\nType: ${query.errorType}\nRequest ID: ${query.requestId||'Not returned'}`}</code>}</div></div>}
