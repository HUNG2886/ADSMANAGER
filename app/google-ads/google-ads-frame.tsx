...
export function GoogleAdsFrame({user,children}:{user:FrameUser;children:React.ReactNode}){
  ...
  return <main className="ga-shell">
    ...
    <aside className={`ga-sidebar ${open?'open':''}`}>
      <div className="ga-brand"><span>DA</span><strong>David Agency MCC Manager</strong><button onClick={()=>setOpen(false)} aria-label="Đóng"><X size={18}/></button></div>
    ...
    <section className="ga-workspace">
      <header className="ga-topbar"><button className="ga-menu" onClick={()=>setOpen(true)} aria-label="Mở menu"><PanelLeft size={20}/></button><a href="/dashboard" className="ga-mobile-brand">David Agency MCC Manager</a><div className="ga-user"><span>{user.name.slice(0,2).toUpperCase()}</span><div><strong>{user.name}</strong><small>{user.email}</small></div><button onClick={logout} aria-label="Đăng xuất"><LogOut size={16}/></button></div></header>
