...
export function CampaignActions({id,status,budget}:{id:string;status:'ENABLED'|'PAUSED'|'REMOVED';budget:number}){
  ...
  async function mutate(body:object,question:string){if(!window.confirm(question))return;setBusy(true);setError('');const response=await fetch('/api/campaigns',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({id,...body})});const payload=await response.json() as {error?:GoogleAdsApiErrorPayload};if(!response.ok){setError(formatGoogleAdsApiError(payload.error,'Google Ads không thể xử lý yêu cầu.'));setBusy(false);return}window.location.reload()}
