...
const updateSchema=z.object({id:z.string().min(1),name:z.string().trim().min(2).max(100).optional(),role:z.enum(['ADMIN','STAFF']).optional(),status:z.enum(['ACTIVE','SUSPENDED']).optional(),password:password.optional(),mccIds:z.array(z.string().min(1)).max(100).optional()});
...
export async function PATCH(request:Request){
  ...
  const current=await prisma.user.findUnique({where:{id:parsed.data.id},select:{id:true,role:true,status:true}});if(!current)return fail('NOT_FOUND','Không tìm thấy user.',404);
