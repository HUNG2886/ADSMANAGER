...
export function StaffPermissions(){
  ...
  return <section className="ga-permission-list">{connections.length>0&&<div className="ga-connection-filter"><label>{tr(locale,'Kết nối Google','Google connection')}<select value={connectionId} onChange={event=>setConnectionId(event.target.value)}>{connections.map(connection=><option value={connection.id} key={connection.id}>{connection.googleEmail}</option>)}</select></label><span>{visibleMccs.length} {tr(locale,'MCC khả dụng','available MCCs')}</span></div>}{message&&<div className="ga-alert success"><ShieldCheck size={18}/><div><strong>{message}</strong></div></div>}{staff.map(member=><article className="ga-panel" key={member.id}><div className="ga-panel-head"><div><h2>{member.name||member.email}</h2><p>{member.email} · {member.status}</p></div><button className="ga-primary" disabled={busy===member.id} onClick={()=>void save(member.id)}><Save size={14}/>{busy===member.id?tr(locale,'Đang lưu...','Saving...'):tr(locale,'Lưu','Save')}</button></div><div className="ga-permission-grid">{visibleMccs.map(mcc=><label key={mcc.id}><input type="checkbox" checked={(selected[member.id]||[]).includes(mcc.id)} onChange={()=>toggle(member.id,mcc.id)}/><span><strong>{mcc.name}</strong><small>{mcc.customerId} · {mcc.connection.googleEmail}</small></span></label>)}</div></article>)}{staff.length===0&&<div className="ga-empty"><ShieldCheck size={28}/><h2>{tr(locale,'Chưa có nhân viên','No staff accounts')}</h2><p>{tr(locale,'Tạo tài khoản nhân viên trước khi gán MCC.','Create a staff account before assigning MCC access.')}</p></div>}</section>
}
